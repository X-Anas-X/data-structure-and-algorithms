...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(data => data[0].toUpperCase() + data.slice(1));
};
...
let biggerThanLuke = (arr) => {
  ...
  let lukeArr = [];
  arr.forEach(data => {
    if (data.name.includes('Luke')) lukeMass = data.mass;
    if (parseInt(data.mass) > parseInt(lukeMass)) lukeArr.push(data.name);
  });
  return lukeArr.join(' - ');
};
